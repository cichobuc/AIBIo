...
| Žiadny AI | AI partner: 1 supervisor + 4 Phase Coordinators + 8 atomic LLM agents (MVP); +2 `code-generator-*` v Phase 2 |
...
- **LLM cost** — supervisor + 4 coordinators + 8 atomic LLM agents (MVP) + multi-source paralel = drahý setup. Mitigation: Haiku pre high-frequency low-reasoning (`explore-coordinator`, `schema-explorer`, `data-profiler`, `docs-keeper`), Sonnet pre reasoning-heavy coordinators a atomic agents, prompt caching. **`max-budget per session` musí byť implementované v Phase 0** — token counter + warning keď session presiahne threshold, inak demo môže stáť $20-30 za run.
...
- **`@anthropic-ai/claude-agent-sdk` stabilita** — závislosť sa presunula na officiálny Claude Agent SDK (nie raw `@anthropic-ai/sdk`). Mitigation: changelog tracking, pin konkrétna verzia v `package.json`, fallback na priamy `query()` call pre high-risk subagentov ak SDK API zmení breaking changes.
...
- **v1.0** *(this version)* — **Migrácia na `@anthropic-ai/claude-agent-sdk` + Claude Code OAuth autentifikácia.** Supervisor a subagenti prešli z manuálneho `messages.create()` + `tool_use` loop na `query()` s `agents: Record<string, AgentDefinition>` parametrom. Approval gate integrovaný cez `canUseTool` callback. MCP server registrovaný cez `createSdkMcpServer()`. Autentifikácia: Claude Code OAuth (`claude login`) — žiadny `ANTHROPIC_API_KEY`, žiadna kreditová fakturácia. `core/agent-sdk/` premenovaný na `core/orchestration/` (interný adapter layer, nie npm package).
- **v0.9** — **MVP scope fix + bezpečnostné opravy.** Translate a Export X2-X8 presunuté do Phase 2 (post-MVP) — MVP je 36 dní namiesto 58. Credentials encryption presunúté z "planned" do Phase P0a (required). `max-budget per session` pridané ako P0 requirement. Scope creep risk prepísaný s explicitnou MVP definíciou. Canonical agent count: supervisor + 9 LLM subagentov; 8 v MVP (`code-generator` je Phase 2); `translate-validator` je deterministický service (nie LLM agent). AIBIO.md v0.9, AINDERSTANDING.md v0.9.
