...
- **Chat history v GlobalChatPanel** — *Implementované:* multi-turn pamäť cez `thread_id` v `chat_messages`. Posledných 20 správ z aktuálneho threadu sa injektuje do supervisor promptu ako `<conversation_history>`. "New conversation" button vygeneruje nový `threadId` (Zustand persist). Staré thready ostávajú v DB — "Load history" UI je mimo MVP scope. Supervisor messages (len `agentName = 'supervisor'`) sa persistujú po každom `stream_end`.
